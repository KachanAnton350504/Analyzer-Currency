...
class ExchangeRates
  ...
  def initialize date=Time.now.strftime("11/18/2015")
    self.date = date
    self.rates = []
    fill_rates
  end
  ...
  def fill_rates
    ...
    self.rates = self.data['Currency'].map if ok?
  end
...
data = {usd: rates.get_value_by_char_code("USD"), 
        rub: rates.get_value_by_char_code("RUB"), 
        eur: rates.get_value_by_char_code("EUR"), 
        date: rates.date, source: rates.source} if rates.ok?
